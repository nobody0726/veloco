...
VL_TEST(http_server_drains_active_connections_after_shutdown)
{
    vl_runtime_t runtime = {0};
    vl_io_t io = {0};
    vl_http_server_t *server = NULL;
    ...
    VL_ASSERT(vl_http_spawn_connection(server, &runtime, &io, sockets[0]) ==
              VL_ERROR_CLOSED);
    ...
    VL_REQUIRE(read(sockets[1], response, sizeof(response) - 1) > 0);
    VL_ASSERT(strstr(response, "HTTP/1.1 200 OK\r\n") != NULL);
    VL_ASSERT(vl_http_server_active_connections(server) == 0);
...
void vl_register_http_server_tests(void)
{
    ...
    vl_test_add("http_server_drains_active_connections_after_shutdown",
                http_server_drains_active_connections_after_shutdown);
}
